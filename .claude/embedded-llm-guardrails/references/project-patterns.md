...
- STM32F1 专项：从 `.ioc`、`main.c`、`stm32f1xx_hal_msp.c`、`stm32f1xx_it.c`、`stm32f1xx_hal_conf.h` 提取 AFIO remap、SWJ/SWD/JTAG 配置、DMA 初始化顺序、NVIC 优先级、IRQHandler 与 HAL handler 映射。

**边界配置要点：**
- `USER CODE` 区域内允许修改，区域外禁止修改
- `.ioc` 文件只读（引导用户通过 CubeMX 操作）
- HAL 驱动和 CMSIS 目录中的既有文件只读；缺少官方库文件时，可经用户确认后从匹配版本官方包复制新增
- CubeMX 重新生成后需检查边界是否需要更新
- STM32F1 项目添加或调整外设时，必须检查 PA13/PA14 SWD、PD0/PD1 HSE、AFIO remap、DMA 通道、IRQ 和优先级表，并更新 `.ai/STM32_RESOURCE_TABLE.md` 或项目已有等效资源表。
...
- CMSIS 和 HAL 驱动目录中的既有文件只读；缺少官方库文件时，可经用户确认后从匹配版本官方包复制新增
...
**边界配置要点：**
- 启动文件和链接脚本只读
- 厂商 SDK/HAL 既有文件只读；缺少官方库文件时，可经用户确认后从匹配版本官方包复制新增
- 应用代码目录允许修改
- 需要用户确认哪些是已验证代码
...
| 资源 | 提取方式 |
|------|----------|
| AFIO remap | 搜索 `__HAL_AFIO_REMAP_`、`GPIO_Remap`、`.ioc` 中的 remap/SWJ 配置 |
| SWD/JTAG | 搜索 `__HAL_AFIO_REMAP_SWJ_`，检查 PA13/PA14 是否被占用 |
| HSE 引脚 | 检查 PD0/PD1 是否作为 HSE 或普通 GPIO/外设复用 |
| DMA | 搜索 `MX_DMA_Init()`、`HAL_DMA_Init()`、`__HAL_LINKDMA()` 和 DMA 通道实例 |
| IRQ | 从 `stm32f1xx_it.c` 提取 `*_IRQHandler()` 和 `HAL_xxx_IRQHandler()` |
| NVIC 优先级 | 搜索 `HAL_NVIC_SetPriority()`、`HAL_NVIC_EnableIRQ()` |
| HAL 模块 | 从 `stm32f1xx_hal_conf.h` 提取 `HAL_xxx_MODULE_ENABLED` |
