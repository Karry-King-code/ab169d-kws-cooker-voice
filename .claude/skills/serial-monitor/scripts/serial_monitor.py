    ...
    try:
        while True:
            ...
            if ser.in_waiting > 0:
                ...
                # 字节级缓冲 + 整行解码：避免数据块边界切断多字节字符(UTF-8/GBK)导致乱码
                line_buffer += data
                while b"\n" in line_buffer:
                    ...
                    # 编码兼容：优先 UTF-8；失败回退 GBK；仍失败则用替换字符
                    try:
                        raw_line = line_bytes.decode("utf-8")
                    except UnicodeDecodeError:
                        try:
                            raw_line = line_bytes.decode("gbk")
                        except UnicodeDecodeError:
                            raw_line = line_bytes.decode("gbk", errors="replace")
                    ...
                    print(display_line)
                    logs.append(LogEntry(raw=raw_line, clean=clean_line, timestamp=time.time()))
